...
class grid_case:
    ...
    def __init__(self,
        ...
        self.observation_space = copy.deepcopy(np.hstack(
            (np.array(self.model.res_bus.vm_pu[[0]+self.id_iber + self.id_svc]), 
             np.array(self.model.res_ext_grid.iloc[0]), 
             np.array(self.model.res_line.i_from_ka.iloc[self.id_line_f_bus]),
