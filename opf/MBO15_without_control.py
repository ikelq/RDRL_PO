...
class grid_case:
    ...
    def __init__(self,
        ...
        if self.env_name == 118:
            self.model = pc.from_mpc('case118zh.mat', f_hz=50, casename_mpc_file='mpc', validate_conversion=False)
            iber_re_capacity = 2
            id_svc_capacity =3
        ...
        for i in self.id_iber:
            pp.create_sgen(self.model, bus=i, p_mw=0, q_mvar=0, name='IBVR', scaling=1.0, in_service=True,
                           max_p_mw=0, min_p_mw=0, max_q_mvar=iber_re_capacity, min_q_mvar=-iber_re_capacity, controllable=True)
        ...
        # self.load_pu = self.load_pu[:110*96]
        # self.gene_pu = self.gene_pu[:110*96]
        # x = np.arange(self.load_pu.shape[0])
        # f_load = interpolate.interp1d(x, self.load_pu, kind="quadratic", axis=0)
        # f_gen = interpolate.interp1d(x, self.gene_pu, kind="quadratic", axis=0)
        # x_new = np.arange(0, self.load_pu.shape[0] - 1, 1 / 15)
        # self.load_pu = f_load(x_new)
        # self.gene_pu = f_gen(x_new)
        ...
        print(self.load_pu.max(), self.load_pu.min(), self.gene_pu.max(), self.gene_pu.min() )

        self.model.bus.min_vm_pu[1:] = 0.8
        self.model.bus.max_vm_pu[1:] = 1.2
        ...
        for i in self.id_iber + self.id_svc:
            self.model.bus.min_vm_pu[i] = 0.95
            self.model.bus.max_vm_pu[i] = 1.05
    ...
    def step_model(self):
        ...
        # pp.runpp(self.model, algorithm='bfsw')
        action = 0*self.model.res_sgen.q_mvar[:self.control_n]
        # print(self.model.res_bus.vm_pu.min(),self.model.res_bus.vm_pu.argmin(),
        # self.model.res_bus.vm_pu.max(),self.model.res_bus.vm_pu.argmax(),self.step_n)
        #
...
class DPGAgent:
    ...
    def train(self, num_frames: int, plotting_interval: int = 400, safepolicy=True, virtualinteraction=True):
        ...
        data_train.to_csv('train' + str(self.env.name) + str(self.seed) + 'vvow'+'.csv')
        ...
        data_train_test.to_csv('traintest' + str(self.env.name) + str(self.seed) + 'vvow'+'.csv')
        ...
        data_inmodel.to_csv('trainstep' + str(self.env.name) + str(self.seed) + 'vvow' + '.csv')

        data_action = np.array(actions)
        np.save('action' + str(self.env.name) + str(self.seed) + 'vvow' + '.npy', data_action)

        data_reward = np.array(rewards)
        np.save('reward' + str(self.env.name) + str(self.seed) + 'vvow' + '.npy', data_reward)
        ...
        datatest_step.to_csv('trainteststep' + str(self.env.name) + str(self.seed) + 'vvow' + '.csv')

        data_time = {"decision_times": decision_times,
        "train_times": train_times
        }
        data_time = DataFrame(data_time)
        data_time.to_csv('data_time' + str(self.env.name) + str(self.seed) + 'vvow' + '.csv')
...
if __name__ == "__main__":
    ...
    for env_name in [env_name]:
        ...
        if env_name == 33:
            ...
            gen_scale = 1.5
        if env_name == 118:
            # id_iber = [33, 50, 53, 68, 73, 97, 107, 111]
            # id_svc = [44, 104]
            id_iber = [33, 44, 50, 53, 76, 97, 106, 111]
            id_svc = [69, 84]
            load_scale = 1
            gen_scale = 2
            # id_iber = [33, 50, 53, 68, 73, 97, 107, 110]
        ...
        agent.train(num_frames, plotting_interval=400000)
        # agent.test(test_frames)
# python MBO15.py --env_name 33 & python MBO15.py --env_name 69 & python MBO15.py --env_name 118